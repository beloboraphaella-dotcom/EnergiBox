from auth import (
    ...
    list_all_users, set_user_suspended, delete_user
)
...
def estimate_bill(home_id: int, authorization: str = Header(...)):
...
@app.get("/alerts")
def get_alerts(home_id: int, authorization: str = Header(...)):
    _scoped_user(authorization, home_id)
    conn = get_raw_db()
    cursor = conn.cursor()
    cursor.execute("""
...
@app.get("/dashboard")
def get_dashboard(home_id: int, authorization: str = Header(...)):
...
@app.get("/schedules")
def get_schedules(home_id: int, authorization: str = Header(...)):
    _scoped_user(authorization, home_id)
    conn = get_raw_db()
    cursor = conn.cursor()
    cursor.execute("""
...
def create_schedule(
    ...
    _verify_home_ownership(user_id, _monitored_point_home_id(monitored_point_id))
    conn = get_raw_db()
    cursor = conn.cursor()
    cursor.execute("""
...
def update_schedule(schedule_id: int, on_time: str = None, off_time: str = None, authorization: str = Header(...)):
    """Edit the on/off times of a manual or accepted-AI schedule"""
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, _schedule_home_id(schedule_id))
...
def delete_schedule(schedule_id: int, authorization: str = Header(...)):
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, _schedule_home_id(schedule_id))
    conn = get_raw_db()
    cursor = conn.cursor()
...
def toggle_schedule(schedule_id: int, authorization: str = Header(...)):
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, _schedule_home_id(schedule_id))
    conn = get_raw_db()
    cursor = conn.cursor()
...
@app.get("/rooms")
def get_rooms(home_id: int, authorization: str = Header(...)):
    _scoped_user(authorization, home_id)
    conn = get_raw_db()
    cursor = conn.cursor()
...
def update_monitored_point(point_id: int, name: str = None, type: str = None, room_id: int = None, authorization: str = Header(...)):
    if type is not None and type not in ("appliance", "socket"):
        raise HTTPException(status_code=400, detail="type must be 'appliance' or 'socket'")

    user_id = _user_id_from_header(authorization)
    home_id = _monitored_point_home_id(point_id)
    _verify_home_ownership(user_id, home_id)
...
def delete_monitored_point(point_id: int, authorization: str = Header(...)):
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, _monitored_point_home_id(point_id))
...
@app.get("/devices")
def get_devices(home_id: int, authorization: str = Header(...)):
...
@app.get("/devices/{mac}")
def get_device_detail(mac: str, authorization: str = Header(...)):
...
def control_device(mac: str, command: str, authorization: str = Header(...)):
...
def _require_admin(authorization: str) -> int:
    """Verify the caller is an authenticated admin. Returns the admin's user_id."""
    if not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Invalid token format")
    payload = decode_token(authorization.replace("Bearer ", ""))
    if not payload:
...
def _scoped_user(authorization: str, home_id: int) -> int:
    """Decode the JWT and verify home_id belongs to the caller. Returns user_id."""
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, home_id)
    return user_id
...
@app.get("/homes")
def get_homes(authorization: str = Header(...)):
...
@app.post("/homes")
def create_home(name: str, address: str = None, authorization: str = Header(...)):
    """Create a new home for the caller"""
    user_id = _user_id_from_header(authorization)
    conn = get_raw_db()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO homes (user_id, name, address) VALUES (%s, %s, %s)", (user_id, name, address))
    conn.commit()
...
def update_home(home_id: int, name: str = None, address: str = None, authorization: str = Header(...)):
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, home_id)
    fields, values = [], []
    if name is not None:
...
def delete_home(home_id: int, authorization: str = Header(...)):
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, home_id)
...
def update_profile(name: str, authorization: str = Header(...)):
...
def update_password(current_password: str, new_password: str, authorization: str = Header(...)):
    """Change the current user's password"""
    user_id = _user_id_from_header(authorization)
    ok, error = change_password(user_id, current_password, new_password)
    if not ok:
        raise HTTPException(status_code=400, detail=error)
    return {"message": "Password updated successfully"}

@app.get("/suggestions")
def get_suggestions(home_id: int, authorization: str = Header(...)):
    _scoped_user(authorization, home_id)
    conn = get_raw_db()
    cursor = conn.cursor()
    cursor.execute("""
...
def accept_suggestion(suggestion_id: int, authorization: str = Header(...)):
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, _suggestion_home_id(suggestion_id))
    conn = get_raw_db()
    cursor = conn.cursor()
...
def ignore_suggestion(suggestion_id: int, authorization: str = Header(...)):
    user_id = _user_id_from_header(authorization)
    _verify_home_ownership(user_id, _suggestion_home_id(suggestion_id))
    conn = get_raw_db()
    cursor = conn.cursor()
...
def get_daily_history(home_id: int, month: int = None, year: int = None, authorization: str = Header(...)):
...
def get_weekly_history(home_id: int, authorization: str = Header(...)):
...
def get_yearly_history(home_id: int, year: int = None, authorization: str = Header(...)):
...
def get_history_by_appliance(home_id: int, month: int = None, year: int = None, authorization: str = Header(...)):
...
def get_overview(home_id: int, authorization: str = Header(...)):
...
def admin_suspend_user(user_id: int, suspended: bool, authorization: str = Header(...)):
    _require_admin(authorization)
    set_user_suspended(user_id, suspended)
    return {"message": "User suspended" if suspended else "User reinstated"}

@app.delete("/admin/users/{user_id}")
def admin_delete_user(user_id: int, authorization: str = Header(...)):
...
@app.get("/admin/stats")
def admin_platform_stats(authorization: str = Header(...)):
    _require_admin(authorization)
    conn = get_raw_db()
    cursor = conn.cursor()
...
def get_rooms_consumption(home_id: int, authorization: str = Header(...)):
